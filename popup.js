...
var setStyle = function (attribute, value) {
  ...
  if (attribute === 'line-height') {
    if (value === "0") {
      ...
      if (DEBUG) {
        console.log("Popup's line height is offset and is being set to: " + newLineHeight);
      }
  ...
  for (var i = 0; i < options.length; i++) {
    ...
    chrome.storage.sync.get(attribute, function (result) {
      if (result) {
        for (var i in result) {
          if (DEBUG) {
            console.log("attribute: " + i + " loaded in menu with value: " + result[i]);
          }
          setStyle(i, result[i]);
  ...
  load.onclick = function () {
    ...
    var loadSaved = function (attribute) {
      chrome.storage.sync.get("saved " + attribute, function (result) {
        ...
        num_styles_loaded++;
        if (num_styles_loaded === STYLE_NAMES.length) {
